...
this.isSubmitted = false; 
 ...
 starttimeref.update({
  ...
  sendmail(){ 
    ...
    this.storageService.getObject('b4toilet form csv').then(result => {
    if (result != null) {
    console.log('b4toilet form csv: '+ result);
    this.idata= result;
    }
    }).catch(e => {
    console.log('error: ', e);
...
this.stdata=this.arrayToCSV(this.arr) ;
   this.storageService.setObject('b4toilet form csv', this.stdata);
...
washingtonRef.update({
  Dining:arrayUnion({
       "b4toiletu": this.DiningData
      
       
     })
}).then(function() {
  console.log("b4toilet data is  updated");
});
