...
export class B4ToiletPage implements OnInit {
  ...
  Save1(): void {
    ...
    console.log('b4toiletdata', JSON.stringify(this.b4toiletdata));
  }
  sendMessage1(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.b4toiletdata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        bedroom4_Toilet: arrayUnion({
          bedroom4_Toilet_data: this.DiningData
        })
      })
      .then(function () {
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.b4toiletdata);

    var csv = this.papa.unparse(this.b4toiletdata);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'bedroom4_Toilet.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
      ...
      a.download = 'bedroom4_Toilet.csv';
