...
export class B3ToiletPage implements OnInit {
  ...
  Save1(): void {
    ...
    console.log('b3toiletdata', JSON.stringify(this.b3toiletdata));
  }
  sendMessage1(): void {
 ...
 sendMessage31(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.b3toiletdata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        bedroom3_Toilet: arrayUnion({
          bedroom3_Toilet_data: this.DiningData
        })
      })
      .then(function () {
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.b3toiletdata);

    var csv = this.papa.unparse(this.b3toiletdata);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'Bedroom3_Toilet.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
      ...
      a.download = 'Bedroom3_Toilet.csv';
