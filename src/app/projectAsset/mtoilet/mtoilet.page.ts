...
interface StudentData {
  ...
  Ceiling_tiles_are_free_of_stains_or_undulations_or_cracks_or_hollowness_etc: string;
  Ceiling_tiles_are_free_of_stains_or_undulations_or_cracks_or_hollowness_etc_Photo: any;
  Ceiling_tiles_are_free_of_stains_or_undulations_or_cracks_or_hollowness_etc_Description: string;
...
export class MToiletPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ionicForm = this.formBuilder.group({
      ...
      Ceiling_tiles_are_free_of_stains_or_undulations_or_cracks_or_hollowness_etc: [''],
      Ceiling_tiles_are_free_of_stains_or_undulations_or_cracks_or_hollowness_etc_Photo: [''],
      Ceiling_tiles_are_free_of_stains_or_undulations_or_cracks_or_hollowness_etc_Description: [''],
  ...
  save1(): void {
    ...
    console.log('mtoiletdata', JSON.stringify(this.mtoiletdata));
  }
  sendMessage1(): void {
  ...
  getGallery32() {
    ...
    this.camera.getPicture(options).then(imageData => {
      ...
      this.ionicForm.get('Wash_Basin_in_mm_Photo').setValue(this.imgURL32);
      this.upload32();
    });
  }
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.mtoiletdata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        masterbedroom_Toilet: arrayUnion({
          masterbedroom_Toilet_Data: this.DiningData
        })
      })
      .then(function () {
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.mtoiletdata);

    var csv = this.papa.unparse(this.mtoiletdata);
