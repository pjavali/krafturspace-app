...
this.cameradisplay31=false;  
 ...
 starttimeref.update({
  ...
  sendmail(){ 
    ...
    this.storageService.getObject('b2balcony form csv').then(result => {
    if (result != null) {
    console.log('b2balcony form csv: '+ result);
    this.idata= result;
    }
    }).catch(e => {
    console.log('error: ', e);
...
this.stdata=this.arrayToCSV(this.arr) ;
   this.storageService.setObject('b2balcony form csv', this.stdata);
...
washingtonRef.update({
  Dining:arrayUnion({
       "b2balconyu": this.DiningData
      
       
     })
}).then(function() {
  console.log("b2balcony data is  updated");
});
