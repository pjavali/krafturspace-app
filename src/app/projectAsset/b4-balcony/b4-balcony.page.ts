...
export class B4BalconyPage implements OnInit {
  ...
  save1(): void {
    ...
    console.log('b4balconydata', JSON.stringify(this.b4balconydata));
  }
  sendMessage1(): void {
 ...
 sendMessage31(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.b4balconydata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        BedRoom4_balcony: arrayUnion({
          BedRoom4_balcony_data: this.DiningData
        })
      })
      .then(function () {
        console.log('BedRoom4_balcony is  updated');
        this.isenabled=false;
      });
  }
  exportCSV() {
    this.storageService.setObject('socialdata', this.b4balconydata);

    var csv = this.papa.unparse(this.b4balconydata);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'BedRoom4_balcony.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
      ...
      a.download = 'BedRoom4_balcony.csv';
