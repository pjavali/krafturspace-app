...
const db =firebase.firestore();   
 ...
 starttimeref.update({
  ...
  sendmail(){ 
    ...
    this.storageService.getObject('b4dress form csv').then(result => {
    if (result != null) {
    console.log('b4dress form csv: '+ result);
    this.idata= result;
    }
    }).catch(e => {
    console.log('error: ', e);
...
this.stdata=this.arrayToCSV(this.arr) ;
   this.storageService.setObject('b4dress form csv', this.stdata);
...
washingtonRef.update({
  Dining:arrayUnion({
       "b4dressu": this.DiningData
      
       
     })
}).then(function() {
  console.log("b4dress data is  updated");
});
