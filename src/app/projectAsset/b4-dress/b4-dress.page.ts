...
export class B4DressPage implements OnInit {
  ...
  Save1(): void {
    ...
    console.log('b4dressdata', JSON.stringify(this.b4dressdata));
  }
  sendMessage1(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.b4dressdata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        BedRoom4_Dress: arrayUnion({
          BedRoom4_Dress_data: this.DiningData
        })
      })
      .then(function () {
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.b4dressdata);

    var csv = this.papa.unparse(this.b4dressdata);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'BedRoom4_Dress.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
