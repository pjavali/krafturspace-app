...
export class Bedroom1Page implements OnInit {
  ...
  Save1(): void {
    ...
    console.log('bedroom1data', JSON.stringify(this.bedroom1data));
  }
  sendMessage1(): void {
 ...
 sendMessage31(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.bedroom1data);
    ...
    washingtonRef
      .update({
        Bedroom1: arrayUnion({
          Bedroom1_data: this.DiningData
        })
      })
      .then(function () {
        console.log('Bedroomm1 is  updated');
        this.isenabled = false;
      });
  }
  exportCSV() {
    this.storageService.setObject('socialdata', this.bedroom1data);

    var csv = this.papa.unparse(this.bedroom1data);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'Bedroom1.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
