...
@Component({
  selector: 'app-b3-balcony',
  templateUrl: './b3-balcony.page.html',
...
export class B3BalconyPage implements OnInit {
  ...
  save1(): void {
    ...
    console.log('b3balconydata', JSON.stringify(this.b3balconydata));
  }
  sendMessage1(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.b3balconydata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        BedRoom3_Balcony: arrayUnion({
          BedRoom3_Balcony_Data: this.DiningData
        })
      })
      .then(function () {
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.b3balconydata);

    var csv = this.papa.unparse(this.b3balconydata);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'BedRoom3_Balcony.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
      ...
      a.download = 'BedRoom3_Balcony.csv';
