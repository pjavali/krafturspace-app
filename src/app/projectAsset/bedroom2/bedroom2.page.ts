...
export class Bedroom2Page implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ionicForm = this.formBuilder.group({
      ...
      Skirting_is_finished_and_aligned_without_gaps: ['', [Validators.required]],
  ...
  Save1(): void {
    ...
    console.log('bedroom2data', JSON.stringify(this.bedroom2data));
  }
  sendMessage1(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.bedroom2data);
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.bedroom2data);

    var csv = this.papa.unparse(this.bedroom2data);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'BedRoom2_data.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
