...
export class B3DressPage implements OnInit {
  ...
  Save1(): void {
    ...
    console.log('b3dressdata', JSON.stringify(this.b3dressdata));
  }
  sendMessage1(): void {
 ...
 sendMessage31(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.b3dressdata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        BedRoom3_Dress: arrayUnion({
          BedRoom3_Dress_data: this.DiningData
        })
      })
      .then(function () {
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.b3dressdata);

    var csv = this.papa.unparse(this.b3dressdata);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'BedRoom3_Dress.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
