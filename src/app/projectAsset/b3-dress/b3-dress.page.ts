...
this.cameradisplay31=false;  
 ...
 starttimeref.update({
  ...
  sendmail(){ 
    ...
    this.storageService.getObject('b3dress form csv').then(result => {
    if (result != null) {
...
this.stdata=this.arrayToCSV(this.arr) ;
   this.storageService.setObject('b3dress form csv', this.stdata);
...
washingtonRef.update({
  Dining:arrayUnion({
       "b3dressu": this.DiningData
      
       
     })
}).then(function() {
  console.log("b3dress data is  updated");
});
