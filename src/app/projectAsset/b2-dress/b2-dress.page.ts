...
const db =firebase.firestore();   
 ...
 starttimeref.update({
  ...
  sendmail(){ 
    ...
    this.storageService.getObject('b2dress form csv').then(result => {
    if (result != null) {
...
this.stdata=this.arrayToCSV(this.arr) ;
   this.storageService.setObject('b2dress form csv', this.stdata);
...
washingtonRef.update({
  Dining:arrayUnion({
       "b2dressu": this.DiningData
      
       
     })
}).then(function() {
  console.log("b2dress data is  updated");
});
