...
export class B2DressPage implements OnInit {
  ...
  Save1(): void {
    ...
    console.log('b2dressdata', JSON.stringify(this.b2dressdata));
  }
  sendMessage1(): void {
...
sendMessage31(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.b2dressdata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        BedRoom2_Dress: arrayUnion({
          BedRoom2_Dress_data: this.DiningData
        })
      })
      .then(function () {
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.b2dressdata);

    var csv = this.papa.unparse(this.b2dressdata);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, ' BedRoom2_Dress.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
