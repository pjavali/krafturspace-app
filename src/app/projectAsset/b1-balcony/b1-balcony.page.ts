...
this.cameradisplay31=false;  
 ...
 starttimeref.update({
  ...
  sendmail(){ 
    ...
    this.storageService.getObject('b1balcony form csv').then(result => {
    if (result != null) {
    console.log('b1balcony form csv: '+ result);
    this.idata= result;
    }
    }).catch(e => {
    console.log('error: ', e);
...
this.stdata=this.arrayToCSV(this.arr) ;
   this.storageService.setObject('b1balcony form csv', this.stdata);
...
washingtonRef.update({
  Dining:arrayUnion({
       "b1balconyu": this.DiningData
      
       
     })
}).then(function() {
  console.log("b1balcony data is  updated");
});
