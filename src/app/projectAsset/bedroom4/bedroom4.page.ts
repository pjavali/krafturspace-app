...
export class Bedroom4Page implements OnInit {
  ...
  Save1(): void {
    ...
    console.log('bedroom4data', JSON.stringify(this.bedroom4data));
  }
  sendMessage1(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.bedroom4data);
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.bedroom4data);

    var csv = this.papa.unparse(this.bedroom4data);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'BedRoom4_data.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
