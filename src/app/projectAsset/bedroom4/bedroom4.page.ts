...
this.cameradisplay25=false;
 ...
 starttimeref.update({
  ...
  sendmail(){ 
    ...
    this.storageService.getObject('bedroom4 form csv').then(result => {
    if (result != null) {
    console.log('bedroom4 form csv: '+ result);
    this.idata= result;
    }
    }).catch(e => {
    console.log('error: ', e);
...
this.stdata=this.arrayToCSV(this.arr) ;
   this.storageService.setObject('bedroom4 form csv', this.stdata);
...
washingtonRef.update({
  Dining:arrayUnion({
       "bedroom4u": this.DiningData
      
       
     })
}).then(function() {
  console.log("bedroom4 data is  updated");
});
