...
this.isSubmitted = false; 
 ...
 starttimeref.update({
  ...
  sendmail(){ 
    ...
    this.storageService.getObject('b2toilet form csv').then(result => {
    if (result != null) {
...
this.stdata=this.arrayToCSV(this.arr) ;
   this.storageService.setObject('b2toilet form csv', this.stdata);
...
washingtonRef.update({
  Dining:arrayUnion({
       "b2toiletu": this.DiningData
      
       
     })
}).then(function() {
  console.log("b2toiletu data is  updated");
});
