...
export class MasterbedroomPage implements OnInit {
  ...
  save1(): void {
    ...
    console.log('masterbedroom', JSON.stringify(this.masterbedroom));
  }
  sendMessage1(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.masterbedroom);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        masterbedroom: arrayUnion({
          masterbedroom_data: this.DiningData
        })
      })
      .then(function () {
        console.log('masterbedroom is  updated');
        this.isenabled = false;
      });
  }
  exportCSV() {
    this.storageService.setObject('socialdata', this.masterbedroom);

    var csv = this.papa.unparse(this.masterbedroom);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'masterbedroom.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
  ...
  Skirting_is_finished_and_aligned_without_gaps(): void {
    let Qvalue = this.ionicForm.get('Skirting_is_finished_and_aligned_without_gaps').value;
    console.log('Q---->', Qvalue);
    if (Qvalue === 'No') {
