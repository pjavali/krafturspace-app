...
export class B1ToiletPage implements OnInit {
  ...
  Save1(): void {
    ...
    console.log('b1toiletdata', JSON.stringify(this.b1toiletdata));
  }
  sendMessage1(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.b1toiletdata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        Bedroom1_Toilet: arrayUnion({
          Bedroom1_Toilet_data: this.DiningData
        })
      })
      .then(function () {
  ...
  exportCSV() {
    this.storageService.setObject('socialdata', this.b1toiletdata);

    var csv = this.papa.unparse(this.b1toiletdata);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'Bedroom1_Toilet.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
