...
this.isSubmitted = false; 
 ...
 starttimeref.update({
  ...
  sendmail(){ 
    ...
    this.storageService.getObject('b1toilet form csv').then(result => {
    if (result != null) {
...
this.stdata=this.arrayToCSV(this.arr) ;
   this.storageService.setObject('b1toilet form csv', this.stdata);
...
washingtonRef.update({
  Dining:arrayUnion({
       "b1toiletu": this.DiningData
      
       
     })
}).then(function() {
  console.log("b1toiletu data is  updated");
});
