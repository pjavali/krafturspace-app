...
export class B1DressPage implements OnInit {
  ...
  Save1(): void {
    ...
    console.log('b1dressdata', JSON.stringify(this.b1dressdata));
  }
  sendMessage1(): void {
  ...
  submitForm(): void {
    ...
    this.AssetData = JSON.stringify(this.b1dressdata);
    this.DiningData = JSON.parse(this.AssetData);
    console.log('formdata collection', this.DiningData);

    washingtonRef
      .update({
        BedRoom1_Dress: arrayUnion({
          BedRoom1_Dress_data: this.DiningData
        })
      })
      .then(function () {
        console.log('BedRoom1_Dress is  updated');
        this.isenabled = false;
      });
  }
  exportCSV() {
    this.storageService.setObject('socialdata', this.b1dressdata);

    var csv = this.papa.unparse(this.b1dressdata);

    if (this.platform.is('cordova')) {
      this.file.writeFile(this.file.dataDirectory, 'BedRoom1_Dress.csv', csv, { replace: true }).then(
        res => {
          this.socialSharing
